...
<img src="kakei_fig/representative/順序尺度.pmg.png" alt="順序尺度.pmg" style="zoom:25%;" />
...
<img src="kakei_fig/dispersion/分散_大小.png" alt="分散_大小" style="zoom:33%;" />

要素数が_n_個のデータ($x_1$, ･･･, $ x_n$)からなる母集団の算術平均値を$\mu$とすると，この母集団の分散である__母分散__$\sigma^2$は以下の式で求められます．
$$
...
センチメートルを単位としたときもインチを単位としたときも共分散は正の値となるので，どちらの単位であっても年齢が増えるほど身長が高くなると言う正の比例関係は共分散から知ることができます．しかし，同一のデータの年齢と身長の関係性を表す数値が単位が違うだけで異なってしまうことは不都合です．
...
z_{i} = py_{i} + q
$$
とします．ただし，$p$および$q$は定数です．

このとき
$$
\bar{z} = \frac{1}{n}\sum_{i = 1}^n z_{i} = \frac{1}{n}\sum_{i = 1}^n (py_{i} + q) 
= \frac{p}{n}\sum_{i = 1}^n y_{i} + q  = p\bar{y} + q となるので\\
...
よって，s_{xz} = p \left \{ \frac{1}{n}\sum_{i = 1}^n (x_i - \bar{x})(y_{i} - \bar{y}) \right \} = p s_{xy} \\
また，s_{z} = \sqrt{\frac{1}{n}\sum_{i = 1}^n (z_{i} - \bar{z})^2} 
= \sqrt{\frac{1}{n}\sum_{i = 1}^n  \left \{ (py_{i} + q) - (p\bar{y} + q) \right \}^2}
= p \sqrt{\frac{1}{n} \sum_{i = 1}^n (y_{i} - \bar{y})^2} = ps_y \\
以上より，r_{xz} = \frac{s_{xz}}{s_{x}s_{y}} = \frac{ps_{xy}}{s_xps_{y}} 
...
$$
s_x = 1.707･･･\\
s_{xy\_C} = 16.50･･･, \ s_{y\_C} = 9.668･･･, \ s_{xy\_I} = 6.499･･･, \ s_{y\_I} = 3.806･･･なので \\
...
$$
となり，相関係数は変数の単位の変換に対して不変であることが確かめられました．

相関係数は2つの変数間の比例的性質の強さを-1から1までの数値によって表します．

頑強ではない 外れ値 ゼロインフレートな分布

相関係数は順序尺度 相関係数がx倍は間違い

相関と因果 相関関係は因果関係の必要条件の1つ
...
- AとBには関係がないが偶然相関が生じた
- AとBは互いに原因と結果の関係にある（ポジティブフィードバック）

相関係数に統計的に意味があるかは要確認



スピアマンの順位相関係数$\gamma_{s}$ or $r_{s}$（https://en.wikipedia.org/wiki/Spearman%27s_rank_correlation_coefficient）
